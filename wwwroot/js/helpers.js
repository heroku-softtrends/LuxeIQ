(function (window) {
    ...
    var Helpers = {
        ...
        init: function () {
            ...
            if ($.ajaxSetup) {
                $.ajaxSetup({
                    timeout: 29000 // sets timeout to 29 seconds
                });
            }
        },
        ...
        showAlert: function (msg, msgType) {
            ...
            if (typeof bootbox != 'undefined') {
                ...
                bootbox.alert(msgType ? "<strong>" + msgType + ":</strong> " + msg : msg);
            ...
            //        });
            //        if (msgType) {
            //            dialogAlert.dialog("option", "title", msgType);
            //        }
        ...
        showAlertWithRedirection: function (msg, msgType, url) {
            if (window.jQuery) {
                ...
                if (typeof bootbox != 'undefined') {
                    ...
                    bootbox.alert({
                        message: msgType ? "<strong> " + msgType + ":</strong> " + msg : msg,
                        callback: function () {
        ...
        showAlertWithFocus: function (msg, msgType, elm) {
            if (window.jQuery) {
                if (msg === null || msg === undefined)
                    return;
                ...
                if (typeof bootbox != 'undefined') {
                    bootbox.alert({
                        message: "<strong> " + msgType + ":</strong> " + msg,
                        callback: function () {
                            setTimeout(function () {
                                var val = $(elm).val();
                                $(elm).focus().val('').val(val + '');
                            }, 0);
                        }
                    })
                }
        ...
        showConfirm: function (msg, yesFn, noFn, yesBtnName, title) {
            ...
            if (typeof bootbox != 'undefined') {
                bootbox.confirm({
                    ...
                    buttons: {
                        confirm: {
                            label: yesBtnName ? yesBtnName : "Yes",
            ...
            if (typeof bootbox != 'undefined') {
                bootbox.confirm({
                    ...
                    buttons: {
                        ...
                        cancel: {
                            label: noBtnName ? noBtnName : 'Cancel',
                            className: 'btn-default'
                        }
                    },
                    callback: function (result) {
        ...
        showErrorMessage: function (message, isRedColor, elmId) {
            var lblElm;
            if (elmId) { lblElm = document.getElementById(elmId); }
            else { lblElm = document.getElementById("lblMessage"); }
            if (lblElm && message) {
        ...
        resetFormValidator: function (form) {
            var formElm;
            if (form) {
                if (typeof form == "object") {
                    ...
                    formElm = $("form[name='" + form + "']");
                }
            }
            else {
                formElm = $('form:visible');
            }

            if (formElm) {
                //formElm.removeData('validator');
                //formElm.removeData('unobtrusiveValidation');
                $(formElm).data('validator', null);
